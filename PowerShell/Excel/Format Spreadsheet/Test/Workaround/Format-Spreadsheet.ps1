...
$outputFilename = "$Path\testServers.xlsx"
...
Catch
{
    #If the above didn't work, we need to convert the spreadsheet to a compatible format first
    ...
    $wb.SaveAs("$outputFilename", [Microsoft.Office.Interop.Excel.XlFileFormat]::xlOpenXMLWorkbook)
    Start-Sleep -Milliseconds 200
    $excel.Quit()
    [System.Runtime.Interopservices.Marshal]::ReleaseComObject($excel) | Out-Null
    Remove-Variable excel

    #Clear the ExcelFile variable and let out new ouput equal that
    Clear-Variable -name ExcelFile

    #Reassign to our new file
    $ExcelFile = $outputFilename
