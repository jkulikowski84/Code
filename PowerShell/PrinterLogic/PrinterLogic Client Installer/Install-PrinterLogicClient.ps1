...
$LogFile = ([System.IO.Path]::GetDirectoryName($myInvocation.MyCommand.Definition)) + "\logfile.txt"
$Transform = ([System.IO.Path]::GetDirectoryName($myInvocation.MyCommand.Definition)) + "\PL2.mst"
...
function Install-PrinterLogicClient
{
    ...
    $p = Start-Process -FilePath "$env:SystemRoot\system32\msiexec.exe" -ArgumentList $params -PassThru
    $p.WaitForExit()
...
function Update-PrinterLogicClient
{
    ...
    Remove-Item -Path "HKLM:\SOFTWARE\PrinterLogic\PrinterInstaller\Classes" -Force
...
Invoke-WebRequest -Uri https://downloads.printercloud.com/client/setup/PrinterInstallerClient.msi -UseBasicParsing -OutFile "$msi"
...
$params = @("refresh")
$p = Start-Process -FilePath "C:\Program Files (x86)\Printer Properties Pro\Printer Installer Client\PrinterInstallerClient.exe" -ArgumentList $params -PassThru
$p.WaitForExit()
