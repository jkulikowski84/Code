...
$PingingPrinters = $FilteredPrinters | Start-RSJob -Throttle 50 -Batch "Test" -ScriptBlock {
    Param($PingItem)
...
$PingingPrinters | Export-Excel -Path "$Path\Exports\Pingingrinters.xlsx" -WorksheetName "Unresponsive Printers" -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow -TableStyle "Light1"
